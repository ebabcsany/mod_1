...
public final class ItemStack extends net.minecraftforge.common.capabilities.CapabilityProvider<ItemStack> implements net.minecraftforge.common.extensions.IForgeItemStack {
   public static final Codec<ItemStack> ITEMSTACK_CODEC = RecordCodecBuilder.create((p_234698_0_) -> {
   ...
   public List<ITextComponent> getTooltip(@Nullable PlayerEntity playerIn, ITooltipFlag advanced) {
      ...
      for(EquipmentSlotType equipmentslottype : EquipmentSlotType.values()) {
         ...
         if (!multimap.isEmpty() && (i & 2) == 0) {
            ...
            for(Entry<Attribute, AttributeModifier> entry : multimap.entries()) {
               ...
               if (flag) {
                  list.add((new StringTextComponent(" ")).append(new TranslationTextComponent("attribute.modifier.equals." + attributemodifier.getOperation().getId(), DECIMALFORMAT.format(d1), new TranslationTextComponent(entry.getKey().func_233754_c_()))).mergeStyle(TextFormatting.DARK_GREEN));
               } else if (d0 > 0.0D) {
                  list.add((new TranslationTextComponent("attribute.modifier.plus." + attributemodifier.getOperation().getId(), DECIMALFORMAT.format(d1), new TranslationTextComponent(entry.getKey().func_233754_c_()))).mergeStyle(TextFormatting.BLUE));
               } else if (d0 < 0.0D) {
                  d1 = d1 * -1.0D;
                  list.add((new TranslationTextComponent("attribute.modifier.take." + attributemodifier.getOperation().getId(), DECIMALFORMAT.format(d1), new TranslationTextComponent(entry.getKey().func_233754_c_()))).mergeStyle(TextFormatting.RED));
               }
            }
         }
      }
      ...
      if (advanced.isAdvanced()) {
         ...
         if (this.hasTag()) {
            list.add((new TranslationTextComponent("item.nbt_tags", this.getTag().keySet().size())).mergeStyle(TextFormatting.DARK_GRAY));
         }
      }
      ...
      if (this.hasTag() && this.tag.contains("AttributeModifiers", 9)) {
         ...
         for(int i = 0; i < listnbt.size(); ++i) {
            ...
            if (!compoundnbt.contains("Slot", 8) || compoundnbt.getString("Slot").equals(equipmentSlot.getName())) {
               Optional<Attribute> optional = Registry.ATTRIBUTE.getValue(ResourceLocation.tryCreate(compoundnbt.getString("AttributeName")));
